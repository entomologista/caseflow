...
class User(db.Model, UserMixin):
    ...
    google_token = db.Column(db.Text, default=None)  # JSON token
...
class Case(db.Model):
    ...
    number = db.Column(db.String, index=True)  # NUP/SEI/PJe etc.
    ...
    def get_or_create_by_refs(db, refs, default_title="Case"):
        # Very simple: pick first ref as number
...
class Event(db.Model):
    ...
    kind = db.Column(db.String)  # email/doc/note/hearing/etc.
    title = db.Column(db.String, default="")
    occurred_on = db.Column(db.DateTime, default=dt.datetime.utcnow)
    raw_id = db.Column(db.String)  # gmail msg id or drive file id
